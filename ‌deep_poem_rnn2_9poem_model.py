...
from keras.layers import LSTM, Activation, Dense, Dropout, Input, Embedding, recurrent,SpatialDropout1D,Conv1D,MaxPooling1D, Flatten
...
# third feature extractor
    inlstm = SpatialDropout1D(0.2)(embedding)
    lstm1 = LSTM(100,dropout=0.2, recurrent_dropout=0.2)(inlstm)
...
# prediction output
    output = Dense(9, activation='softmax')(merge)
...
model = sharedIP_cnn_lstm(max_len,max_words)
#model = modelbuild()
model.summary()
model.compile(loss='categorical_crossentropy',optimizer='adam',metrics=['accuracy'])

history =model.fit(sequences_matrix,trainLabels,batch_size=32,epochs=10,
...
plot_model(model, to_file='poemmodel.png', show_shapes=True)
...
with open("poemmodel.json", "w") as json_file:
    json_file.write(model_json)

# serialize weights to HDF5
model.save_weights("poemmodel.h5")
